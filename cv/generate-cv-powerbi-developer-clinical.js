// Oyster — Senior Analyst, Finance Operations (Cost Controlling).
// Styled like the cv-analityk website (dark "Matrix" theme, blue/navy, binary rain,
// Inter + JetBrains Mono, skill bars, terminal, AI-project cards, success stories).
// Builds: oyster_web_preview.html (full web CV), the 2-page PDF (<=2MB, role title links
// to the cv-analityk site), and a short webm of the moving background.
// Truthful: NO Python / dbt / Snowflake / ERP / RLS claimed.
...
const WEB_PREVIEW = path.join(outputDir, 'powerbi_developer_web_preview.html');
const RENDER_HTML = path.join(outputDir, '_powerbi_developer_render.html');
const PDF_FILE = path.join(outputDir, 'AG_Power_BI_Developer_Clinical.pdf');
const VIDEO_FILE = path.join(outputDir, 'AG_Power_BI_Developer_CV_background.webm');

const WEB_URL = 'https://adrwis.github.io/cv-analityk/';
const HEADLINE = 'Power BI dashboards that turn complex clinical and operational data into decisions: advanced DAX, SQL, Power Query.';
...
<title>CV — Adriana Guściora · Senior Analyst, Finance Operations</title>
...
<style>
  ...
  /* cv2 + c3 now INCLUDED in the PDF, compacted */
...
<div class="cv-analyst" id="cv-analyst" data-mode="dark">
  ...
  <nav class="cv-analyst__topbar" aria-label="bar">
    <span class="cv-analyst__topbar-title">Adriana Guściora | Senior Analyst · Finance Operations</span>
  ...
  <main class="cv-analyst__container">
    ...
    <section class="cv-analyst__section" id="target-role">
      <div class="cv-analyst__section-header"><div class="cv-analyst__section-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg></div><h2 class="cv-analyst__section-title">Target role · Power BI Developer (Clinical Operations)</h2></div>
      <p style="color:var(--ca-text-muted);max-width:72ch">100% remote (B2B) fit: <strong>Power BI with advanced DAX</strong>, compelling data visualization, and <strong>strong SQL</strong> (writing and optimizing complex queries), with Power Query (M) and <strong>Jira</strong>. I translate business and technical requirements into intuitive dashboards and collaborate on data models. Healthcare-adjacent background (Zimmer Biomet medical devices, ISS pharma client) suits clinical operations.</p>
    </section>
    ...
    <section class="cv-analyst__section" id="core-skills">
      ...
      <div class="cv-analyst__skills-grid">
        <div class="cv-analyst__skill-group cv-analyst__skill-group--ai">
          ...
          ${skillBar('Data Visualization', 90, 'expert')}
        ...
        <div class="cv-analyst__skill-group">
          <h3><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M18 20V10M12 20V4M6 20v-6"/></svg><span>SQL &amp; Data</span></h3>
          ${skillBar('SQL (querying &amp; optimization)', 82, 'advanced')}
          ${skillBar('Data Modeling (star schema)', 82, 'advanced')}
        ...
        <div class="cv-analyst__skill-group">
          ...
          ${skillBar('Stakeholder &amp; Requirements', 85, 'advanced')}
      ...
      <div class="cv-analyst__terminal">
        ...
        <div class="cv-analyst__terminal-body">
          ...
          <div><span class="comment">&gt; Building DAX measures &amp; model...</span></div>
          <div><span class="output">&#10003; Data validated, single source of truth</span></div>
          <div><span class="output">&#10003; Dashboard published &amp; automated</span></div>
...
(async () => {
  ...
  try { fs.rmSync(path.join(repoRoot, 'oyster'), { recursive: true, force: true }); } catch {}
  const fileUrl = 'file:///' + RENDER_HTML.replace(/\\/g, '/');

  const browser = await chromium.launch();

  // PDF — 2-page, dark, even light rain, cv2+cv3 included, static headline
  ...
  if (process.env.SHOT) await page.screenshot({ path: path.join(outputDir, '_oyster_check.png'), fullPage: true });
  ...
  const tmpVideoDir = path.join(outputDir, '_vid_tmp');
